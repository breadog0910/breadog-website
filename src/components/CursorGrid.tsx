...
export default function CursorGrid({
  ...
  useEffect(() => {
    ...
    const energize = (x: number, y: number, boost?: number) => {
      ...
      const ease =
        FALLOFF_CURVES[p.falloff ?? "smooth"] ?? FALLOFF_CURVES.linear;
    ...
    const draw = (now: number) => {
      ...
      const [cr, cg, cb] = hexToRgb(p.color ?? "#6366f1");
      ...
      for (let pi = pulses.length - 1; pi >= 0; pi--) {
        ...
        const minCol = Math.max(
          0,
          Math.floor((pulse.x - ringR - band - offX) / cs)
        );
        ...
        const minRow = Math.max(
          0,
          Math.floor((pulse.y - ringR - band - offY) / cs)
        );
    ...
    // Listen on window so events aren't blocked by higher z-index elements
